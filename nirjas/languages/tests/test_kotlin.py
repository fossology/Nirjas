#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Contract tests for Kotlin extractor.
...
class KotlinTest(unittest.TestCase):
    """Contract tests for Kotlin language support."""

    testfile = os.path.join(
        os.path.abspath(os.path.dirname(__file__)),
        "TestFiles/textcomment.kts",
    )

    def test_output_contract(self):
        """Verify extractor schema and metadata invariants."""

        scan_result = kotlin.kotlinExtractor(self.testfile).get_dict()
        assert_scan_output_contract(self, scan_result, self.testfile, 'Kotlin')

    def test_source_contract(self):
        """Verify source extraction API contract."""

        assert_source_extractor_contract(self, kotlin.kotlinSource, self.testfile)
