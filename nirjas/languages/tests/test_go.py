#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Contract tests for Go extractor.
...
class GoTest(unittest.TestCase):
    """Contract tests for Go language support."""

    testfile = os.path.join(
        os.path.abspath(os.path.dirname(__file__)),
        "TestFiles/textcomment.go",
    )

    def test_output_contract(self):
        """Verify extractor schema and metadata invariants."""

        scan_result = go.goExtractor(self.testfile).get_dict()
        assert_scan_output_contract(self, scan_result, self.testfile, 'Go')

    def test_source_contract(self):
        """Verify source extraction API contract."""

        assert_source_extractor_contract(self, go.goSource, self.testfile)
