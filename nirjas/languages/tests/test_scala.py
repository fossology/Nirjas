#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Contract tests for Scala extractor.
...
class ScalaTest(unittest.TestCase):
    """Contract tests for Scala language support."""

    testfile = os.path.join(
        os.path.abspath(os.path.dirname(__file__)),
        "TestFiles/textcomment.scala",
    )

    def test_output_contract(self):
        """Verify extractor schema and metadata invariants."""

        scan_result = scala.scalaExtractor(self.testfile).get_dict()
        assert_scan_output_contract(self, scan_result, self.testfile, 'Scala')

    def test_source_contract(self):
        """Verify source extraction API contract."""

        assert_source_extractor_contract(self, scala.scalaSource, self.testfile)
